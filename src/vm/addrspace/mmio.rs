...
use crate::{AccessWidth, GuestMemory, GuestMmio, GuestPhysAddr};
...
pub(crate) struct Inner {
    pub regions: BTreeMap<u32, GuestMmio>,
}
...
impl MmioRegions {
    pub fn add_region(&mut self, region: GuestMmio) {
        let inner = unsafe { &mut *self.inner.get() };
        inner.regions.insert(region.dev_id, region);
    }
