...
use crate::{
    AxVMConfig, RunError, VmMachineUninitOps, VmStatusInitOps, VmStatusRunningOps,
...
impl VmDataInner {
    pub fn new(config: AxVMConfig) -> Self {
        Self {
            ...
            machine: RwLock::new(VmMachineState::Uninit(config)),
    ...
    pub fn wait(&self) -> anyhow::Result<()> {
        loop {
            {
                let status_guard = self.machine.read();
                if let VmMachineState::Stopped = &*status_guard {
                    break;
                }
            }
    ...
    pub(crate) fn run_result(&self) -> Result<(), RunError> {
        let mut guard = self.error.write();
        let res = guard.clone();
        match res {
            Some(err) => match err {
                RunError::Exit => Ok(()),
                RunError::ExitWithError(error) => Err(error),
            },
            None => Ok(()),
        }
    }
...
impl VmData {
    ...
    pub fn init(&self) -> anyhow::Result<()> {
        let mut status_guard = self.machine.write();
        match core::mem::replace(&mut *status_guard, VmMachineState::Switching) {
            VmMachineState::Uninit(config) => {
                let vm_init = VmMachineUninit::new(config);
                let init = vm_init.init(self.downgrade())?;
